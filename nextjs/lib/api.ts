...
class ProductsAPI {
  ...
  private mapFromBackend(raw: Record<string, unknown>): Product {
    ...
    const p: Record<string, unknown> = {
      ...
      priceSmall: (raw.priceSmall ?? raw.price_small) as number,
      priceMedium: (raw.priceMedium ?? raw.price_medium) as number,
      priceLarge: (raw.priceLarge ?? raw.price_large) as number,
      stockSmall: (raw.stockSmall ?? raw.stock_small) as number,
      stockMedium: (raw.stockMedium ?? raw.stock_medium) as number,
      stockLarge: (raw.stockLarge ?? raw.stock_large) as number,
      imageUrl: raw.imageUrl ?? raw.image_url ?? raw.image ?? '',
      gallery1: galleryArr[0] ?? raw.gallery_1 ?? raw.gallery1 ?? '',
      gallery2: galleryArr[1] ?? raw.gallery_2 ?? raw.gallery2 ?? '',
      gallery3: galleryArr[2] ?? raw.gallery_3 ?? raw.gallery3 ?? '',
      gallery4: galleryArr[3] ?? raw.gallery_4 ?? raw.gallery4 ?? '',
      gallery5: galleryArr[4] ?? raw.gallery_5 ?? raw.gallery5 ?? '',
